    ...
    <Link href={`/courses/${course.id}`} className="block group">
      <div className="bg-white/[0.02] rounded-2xl overflow-hidden border border-white/10 hover:border-white/20 transition-all duration-300 h-full flex flex-col">
        ...
        <div className="relative aspect-video overflow-hidden">
          ...
          {/* Member Level Badge */}
          <div className={`absolute top-3 left-3 px-2.5 py-1 rounded-lg text-[10px] font-bold flex items-center gap-1 ${
            course.memberLevel === 'VIP' ? 'bg-gradient-to-r from-yellow to-amber-500 text-black' :
            course.memberLevel === 'Premium' ? 'bg-red text-white' :
            'bg-white/90 text-black'
          }`}>
