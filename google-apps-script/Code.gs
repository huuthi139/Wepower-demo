...
// AUTO-SYNC CONFIG
// ==========================================
var WEBAPP_URL = 'https://wedu.vn'; // URL của web app (Vercel)
...
function setup() {
  ...
  if (!users) {
    ...
    // Tạo admin mặc định
    users.appendRow(['admin@wedu.vn', '123456', 'Admin', 'Admin WEDU', 'VIP', '', '', '']);
    Logger.log('Tab Users đã tạo + admin mặc định: admin@wedu.vn / 123456');
  }
...
function doGet(e) {
  try {
    ...
    if (!action) {
      return jsonResponse_({ success: true, message: 'WEDU App API is running', actions: ['ping', 'login', 'register', 'getCourses', 'appendOrder', 'getUsers', 'getUsersForSync', 'updateUserLevel', 'updatePassword', 'deleteUser', 'saveChapters', 'getChapters', 'getAllChapters', 'enrollCourse', 'getEnrollments', 'updateProgress', 'saveReview', 'getReviews'] });
    }

    switch (action) {
      case 'ping':
        return jsonResponse_({ success: true, status: 'ok', app: 'WEDU App' });
...
function seedCourses() {
  ...
  var courses = [
    ...
    [
      ...
      'Lộ trình chi tiết từ 0 đến doanh thu 1 triệu USD! Đây là khóa học flagship của WEDU - tổng hợp 10 năm kinh nghiệm kinh doanh online. Bạn sẽ học cách chọn ngách, xây brand, tạo sản phẩm, marketing đa kênh, tối ưu vận hành và mở rộng quy mô. Kèm theo bộ công cụ và template business plan hoàn chỉnh.',
...
function testConnection() {
  ...
  // Test login
  var loginResult = handleLogin_({ email: 'admin@wedu.vn', password: '123456' });
